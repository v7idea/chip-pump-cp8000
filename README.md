...
- Arduino IDE on macOS does not inherit Terminal environment variables. Put the
  native toolchain at `~/.cp8000/toolchain`, or make that path a symlink to the
  native XuanTie install directory.
...
- [x] Decide release version format. Use Arduino-compatible semver such as
  `0.1.0-alpha.2`, then keep `platform.txt`, archive name, GitHub tag, and
...
- Generated archive: `package/dist/chippump-cp8000-0.1.0-alpha.2.tar.gz`.
- Generated package index: `package/package_chip-pump_cp8000_index.json`.
- Archive SHA-256: generated by `scripts/generate_package_index.py`.
...
- Installed-package Blink compile passed with
  `~/.cp8000/toolchain` on macOS, matching Arduino IDE GUI behavior.
...
```text
https://github.com/v7idea/chip-pump-cp8000/releases/download/0.1.0-alpha.2/package_chip-pump_cp8000_index.json
```
