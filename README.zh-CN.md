...
CP8000 uploader 使用 Python `pyserial` 访问 serial port。从 `0.1.1-alpha.4`
...
如果 Arduino IDE 没有显示最新 CP8000 版本，请关闭 Arduino IDE，并删除以下缓存文件：
...
然后重新打开 Arduino IDE，并检查 Boards Manager 的版本下拉菜单。
...
这个套件目前是 alpha release，用于硬件 bring-up 和 Arduino 生态验证。
